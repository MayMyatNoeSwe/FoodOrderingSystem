            ...
            <header class="bg-white/90 backdrop-blur-md sticky top-0 z-30 border-b border-slate-200/80 px-6 py-4 flex items-center justify-between gap-4 shadow-sm">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-500 hidden sm:block">Master view of individual ordered food items across all customer transactions</p>
                    </div>
                </div>

                <div class="flex items-center gap-3">
                    <a href="{{ route('admin.orders.index') }}" class="px-3.5 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold rounded-xl border border-slate-200 transition-all flex items-center gap-2">
            ...
            <main class="flex-1 p-4 sm:p-6 space-y-6 overflow-y-auto">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                    ...
                    <div class="bg-white border border-slate-200/80 rounded-2xl p-5 relative overflow-hidden group hover:border-slate-300 hover:shadow-md transition-all shadow-sm">
                        <div class="flex items-center justify-between">
                            <span class="text-slate-500 text-xs font-bold uppercase tracking-wider">Total Items Sold</span>
                        ...
                        <div class="text-3xl font-black text-slate-900 mt-2">{{ number_format($totalQuantitySold) }}</div>
                        <div class="text-xs text-slate-500 font-medium mt-2">Dishes delivered across orders</div>
                    </div>

                    <!-- Stat 2: Total Items Value -->
                    <div class="bg-white border border-slate-200/80 rounded-2xl p-5 relative overflow-hidden group hover:border-slate-300 hover:shadow-md transition-all shadow-sm">
                        <div class="flex items-center justify-between">
                            <span class="text-slate-500 text-xs font-bold uppercase tracking-wider">Gross Items Revenue</span>
                        ...
                        <div class="text-2xl font-black text-slate-900 mt-2 truncate">{{ number_format($totalItemsRevenue) }} <span class="text-xs text-orange-600 font-bold">MMK</span></div>
                        <div class="text-xs text-slate-500 font-medium mt-2">Accumulated food item subtotals</div>
                    </div>

                    <!-- Stat 3: Top Selling Dish -->
                    <div class="bg-white border border-slate-200/80 rounded-2xl p-5 relative overflow-hidden group hover:border-slate-300 hover:shadow-md transition-all shadow-sm">
                        <div class="flex items-center justify-between">
                            <span class="text-slate-500 text-xs font-bold uppercase tracking-wider">Top Selling Dish</span>
                        ...
                        <div class="text-lg font-black text-amber-700 mt-2 truncate" title="{{ $topItemName }}">{{ $topItemName }}</div>
                        <div class="text-xs text-slate-500 font-medium mt-2">Most frequently ordered food</div>
                    </div>

                    <!-- Stat 4: Unique Order Items Count -->
                    <div class="bg-white border border-slate-200/80 rounded-2xl p-5 relative overflow-hidden group hover:border-slate-300 hover:shadow-md transition-all shadow-sm">
                        <div class="flex items-center justify-between">
                            <span class="text-slate-500 text-xs font-bold uppercase tracking-wider">Total Item Records</span>
                        ...
                        <div class="text-3xl font-black text-blue-600 mt-2">{{ number_format($orderItems->total()) }}</div>
                        <div class="text-xs text-slate-500 font-medium mt-2">Individual itemized entries</div>
                    </div>
                ...
                <div class="bg-white border border-slate-200/80 rounded-2xl p-5 sm:p-6 shadow-sm space-y-6">
                    ...
                    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                        <div>
                            <h3 class="text-lg font-black text-slate-900 tracking-tight">Order Items Master Table</h3>
                            <p class="text-slate-500 text-xs mt-0.5">Filter by dish name, category, or customer order number</p>
                        </div>
                        ...
                        <form method="GET" action="{{ route('admin.orderItems.index') }}" class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                            ...
                            <select name="category_id" onchange="this.form.submit()" class="bg-slate-50 border border-slate-200 focus:border-orange-500 focus:bg-white text-slate-800 text-xs rounded-xl px-3.5 py-2.5 focus:ring-0 transition-all cursor-pointer">
                            ...
                            @if($search || $categoryId || $status)
                                <a href="{{ route('admin.orderItems.index') }}" class="px-3.5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-xl border border-slate-200 flex items-center justify-center gap-1">
                                    <span>✕</span>
                                    <span>Reset</span>
                                </a>
                            @endif
                            ...
                            <tbody class="divide-y divide-slate-100 text-slate-700 font-medium">
                                @forelse($orderItems as $item)
                                    ...
                                    <tr class="hover:bg-slate-50 transition-colors">
                                        ...
                                        <td class="px-4 py-4">
                                            <div class="flex items-center gap-3">
                                                <div class="w-10 h-10 rounded-xl bg-white border border-slate-200 overflow-hidden shrink-0 flex items-center justify-center text-slate-400 font-bold text-xs">
                                                    @if($dishImage)
                                                ...
                                                <div>
                                                    <div class="font-bold text-slate-900 text-xs">{{ $dishName }}</div>
                                                    <div class="text-[11px] text-slate-500 font-mono mt-0.5">{{ number_format($unitPrice) }} MMK</div>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <!-- Category -->
                                        <td class="px-4 py-4">
                                            <span class="px-2.5 py-1 bg-slate-100 border border-slate-200 text-slate-700 text-[10px] font-bold rounded-lg inline-block">
                                                {{ $categoryName }}
                                            </span>
                                        </td>
                                        ...
                                        <td class="px-4 py-4">
                                            @if($order)
                                                <a href="{{ route('admin.orders.index', ['search' => $orderNum]) }}" class="font-mono text-xs font-black text-orange-600 hover:underline">
                                                    #{{ $orderNum }}
                                                </a>
                                            ...
                                            @else
                                                <span class="text-slate-400">Deleted Order</span>
                                            @endif
                                        </td>
                                        ...
                                        <td class="px-4 py-4 text-center">
                                            <span class="px-2.5 py-1 bg-slate-100 border border-slate-200 rounded-lg text-slate-800 font-bold text-xs">
                                        ...
                                        <td class="px-4 py-4 text-right">
                                            <div class="font-black text-slate-900 text-xs">
                                                {{ number_format($itemSubtotal) }} <span class="text-[10px] text-orange-600 font-bold">MMK</span>
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-4 py-4 text-right">
                                            <form method="POST" action="{{ route('admin.orderItems.destroy', $item) }}" onsubmit="return confirmDeleteOrderItem(this, '{{ addslashes($dishName) }}', '{{ $orderNum }}')">
                                                ...
                                                <button type="submit" title="Delete Item Record" class="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition-colors cursor-pointer border border-transparent hover:border-red-100">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="8" class="px-6 py-12 text-center text-slate-500">
                                            ...
                                            <div class="font-bold text-slate-800">No order items found</div>
