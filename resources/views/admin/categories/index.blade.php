            ...
            <header class="bg-white/90 backdrop-blur-md sticky top-0 z-30 border-b border-slate-200/80 px-6 py-4 flex items-center justify-between gap-4 shadow-sm">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-500 hidden sm:block">Organize menu catalog categories and item classifications</p>
                    </div>
                </div>
            ...
            <main class="flex-1 p-4 sm:p-6 space-y-6 overflow-y-auto">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                    ...
                    <div class="bg-white border border-slate-200/80 rounded-2xl p-5 relative overflow-hidden group hover:border-slate-300 hover:shadow-md transition-all shadow-sm">
                        <div class="flex items-center justify-between">
                            <span class="text-slate-500 text-xs font-bold uppercase tracking-wider">Total Categories</span>
                        ...
                        <div class="text-3xl font-black text-slate-900 mt-2">{{ $categories->total() }}</div>
                    ...
                    <div class="bg-white border border-slate-200/80 rounded-2xl p-5 relative overflow-hidden group hover:border-slate-300 hover:shadow-md transition-all shadow-sm">
                        <div class="flex items-center justify-between">
                            <span class="text-slate-500 text-xs font-bold uppercase tracking-wider">Linked Food Items</span>
                        ...
                        <div class="text-xs text-slate-500 font-medium mt-2">Mapped to categories</div>
                    </div>

                    <!-- Metric Card 3: Filter / Active State Summary -->
                    <div class="bg-white border border-slate-200/80 rounded-2xl p-5 relative overflow-hidden group hover:border-slate-300 hover:shadow-md transition-all shadow-sm sm:col-span-2 lg:col-span-1">
                        <div class="flex items-center justify-between">
                            <span class="text-slate-500 text-xs font-bold uppercase tracking-wider">Search Filter Status</span>
                        ...
                        <div class="text-lg font-bold text-slate-900 mt-2 truncate">
                            @if($search)
                                Filtered: "<span class="text-orange-600">{{ $search }}</span>"
                            @else
                                Showing All Categories
                            @endif
                        </div>
                        <div class="text-xs text-slate-500 font-medium mt-2 flex items-center justify-between">
                            <span>Page {{ $categories->currentPage() }} of {{ max(1, $categories->lastPage()) }}</span>
                            @if($search)
                                <a href="{{ route('admin.categories.index') }}" class="text-orange-600 hover:text-orange-700 font-bold underline text-[11px]">Clear Filter</a>
                            @endif
                        </div>
                    </div>
                ...
                <div class="bg-white border border-slate-200/80 rounded-2xl p-5 sm:p-6 shadow-sm space-y-6">
                    ...
                    <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
                        <div>
                            <h3 class="text-lg font-black text-slate-900 tracking-tight">Category List</h3>
                            <p class="text-slate-500 text-xs mt-0.5">Manage category titles, slugs, and menu assignments</p>
                        </div>
                                ...
                                @forelse($categories as $category)
                                    ...
                                    <tr class="hover:bg-slate-50 transition-colors">
                                        ...
                                        <td class="px-4 py-4 font-bold text-slate-900">
                                            <div class="flex items-center gap-3">
                                                <div class="w-8 h-8 rounded-lg bg-orange-50 border border-orange-100 text-orange-600 flex items-center justify-center text-sm shadow-sm shrink-0">
                                                    {{ $icon }}
                                                </div>
                                        ...
                                        <td class="px-4 py-4 font-mono text-[11px]">
                                            <span class="px-2.5 py-1 bg-slate-100 rounded-md border border-slate-200 text-slate-600 font-semibold inline-block">
                                                {{ $category->slug }}
                                            </span>
                                        </td>
                                        ...
                                        <!-- Items Count -->
                                        <td class="px-4 py-4">
                                            <span class="px-2.5 py-1 bg-amber-50 text-amber-700 rounded-full border border-amber-200 text-[11px] font-bold inline-flex items-center gap-1.5">
                                                <span>{{ $category->menu_items_count }}</span>
                                                <span class="text-amber-600/80 font-normal">Food Items</span>
                                            </span>
                                        </td>
                                    ...
                                    <tr>
                                        <td colspan="6" class="px-4 py-12 text-center text-slate-500">
                                            <div class="max-w-xs mx-auto space-y-3">
                                                ...
                                                <div class="font-bold text-slate-800 text-sm">No Food Categories Found</div>
                                                <p class="text-xs text-slate-500">
                                                    @if($search)
                                                        No category matching "<span class="text-orange-600">{{ $search }}</span>". Try clearing your search keyword.
                                                    @else
                                                        Get started by creating your first food category for your restaurant menu.
                                                    @endif
                                                </p>
                                                @if($search)
                                                    <a href="{{ route('admin.categories.index') }}" class="inline-block px-4 py-2 bg-slate-100 text-slate-700 text-xs font-bold rounded-xl border border-slate-200 hover:bg-slate-200">Clear Search</a>
                                                @else
        ...
        <div @click.outside="createModalOpen = false" 
            ...
            <div class="flex items-center justify-between border-b border-slate-100 pb-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="text-lg font-black text-slate-900">Create New Category</h3>
                        <p class="text-slate-500 text-xs">Add a new food category to organize menu items</p>
                    </div>
                </div>
            ...
            <form method="POST" action="{{ route('admin.categories.store') }}" class="space-y-5">
                ...
                <div class="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-1">
                    ...
                    <div class="font-mono text-xs text-orange-600 font-bold truncate">
            ...
            <div class="flex items-center justify-between border-b border-slate-100 pb-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-slate-500 text-xs">Update existing food category details</p>
                    </div>
                </div>
